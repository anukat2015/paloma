<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@page import="com.paloit.paloma.google.GoogleAuthHelper"%>
<html>
<head>
<link rel="stylesheet"
	href='<c:url value="/resources/bootstrap-3.3.5-dist/bootstrap-3.3.5-dist/css/bootstrap.min.css"/>'/>
<link rel="stylesheet"
	href='<c:url value="/resources/style.css"/>'/>
<script
	src="https://cdnjs.cloudflare.com/ajax/libs/babel-core/5.8.23/browser.min.js"></script>
<script src='<c:url value="/resources/jquery-2.1.4.js"/>'></script>
<script src='<c:url value="/resources/react-0.14.2/build/react.js"/>'></script>
<script src='<c:url value="/resources/react-0.14.2/build/react-dom.js"/>'></script>
<script src='<c:url value="/resources/bootstrap-3.3.5-dist/bootstrap-3.3.5-dist/js/bootstrap.min.js"/>'></script>
<script type="text/babel" src='<c:url value="/resources/profile.js"/>'></script>
<script type="text/babel" src='<c:url value="/resources/main.js"/>'></script>


</head>
<body>
	<div id="main"></div>
</body>
</html>
